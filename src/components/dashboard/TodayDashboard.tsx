...
const focusWindows = [
  { start: new Date(new Date().setHours(9, 0, 0, 0)), end: new Date(new Date().setHours(11, 30, 0, 0)), type: 'deep' as const },
  { start: new Date(new Date().setHours(14, 0, 0, 0)), end: new Date(new Date().setHours(16, 0, 0, 0)), type: 'shallow' as const },
]
...
export function TodayDashboard({
  ...
  const completedToday = tasks.filter(t => t.status === 'completed' && t.completedAt && t.completedAt.toDateString() === new Date().toDateString())
  const progress = focusTimePlanned > 0 ? (focusTimeCompleted / focusTimePlanned) * 100 : 0

  const currentTask = todayTasks.find(t => t.scheduledAt && t.scheduledAt <= new Date() && (!t.completedAt || t.completedAt > new Date())) || todayTasks[0]
  ...
  const aiSummary = React.useMemo(() => {
    const highPriority = todayTasks.filter(t => t.priority === 'high' || t.priority === 'critical').length
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-heading text-3xl font-semibold tracking-tight">{getGreeting()}, Alex</h1>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-4 p-4 rounded-xl bg-primary/5 border border-primary/10">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-heading text-lg font-semibold truncate">{currentTask.title}</h3>
                      <p className="text-sm text-muted-foreground mt-1">{currentTask.description}</p>
                      <div className="flex flex-wrap items-center gap-3 mt-3 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{Math.round(currentTask.estimatedEffort/60)}h {currentTask.estimatedEffort%60}m</span>
                        <span className="flex items-center gap-1"><Zap className="h-3 w-3" />{currentTask.energyRequired}</span>
                        <span className="flex items-center gap-1"><TrendingUp className="h-3 w-3" />AI: {currentTask.aiPriorityScore}%</span>
                      </div>
                    </div>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="rounded-lg bg-muted/50 p-3">
                    <div className="font-heading text-2xl font-bold text-primary">{todayTasks.length}</div>
                    <div className="text-xs text-muted-foreground">Tasks Today</div>
                  </div>
                  <div className="rounded-lg bg-muted/50 p-3">
                    <div className="font-heading text-2xl font-bold text-green-400">{Math.round(progress)}%</div>
                    <div className="text-xs text-muted-foreground">Focus Progress</div>
                  </div>
                  <div className="rounded-lg bg-muted/50 p-3">
                    <div className="font-heading text-2xl font-bold text-amber-400">{completedToday.length}</div>
            ...
            <CardContent className="pt-0">
              <form onSubmit={handleQuickAddSubmit} className="space-y-3">
                <div className="relative">
                  ...
                  />
                  <Button type="submit" size="icon" className="absolute right-2 top-1/2 -translate-y-1/2" aria-label="Add task">
                    <Plus className="h-4 w-4" aria-hidden="true" />
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground">
                  Try: "Review PR #247 by tomorrow, high priority, 45 min" or "Write blog post by next week, medium, 2 hours"
                </p>
              </form>
            </CardContent
          </Card>
            ...
            <CardContent className="pt-0">
              <div className="space-y-3 text-sm">
                <div className="rounded-lg bg-blue-500/10 border border-blue-500/20 p-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Aligned with your balanced energy window (forecast: 65% at 2pm → 85% at 11:45am)</p>
                </div>
                <div className="rounded-lg bg-green-500/10 border border-green-500/20 p-3">
                  <p className="font-medium text-green-400">Added 15min recovery break after client call</p>
                  <p className="text-xs text-muted-foreground mt-1">High-stress meetings reduce subsequent focus quality by ~23%</p>
                </div>
                <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-3">
                  <p className="font-medium text-amber-400">Deferred "Tax documents" to Friday 2pm</p>
                  <p className="text-xs text-muted-foreground mt-1">Low-energy compliance task, batched with other admin work</p>
                </div>
              </div>
            </CardContent
          </Card>
          ...
          <CardContent className="pt-0">
            <div className="space-y-4">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">{Math.round(progress)}% ({focusTimeCompleted}min / {focusTimePlanned}min)</span>
              </div>
              <Progress value={Math.min(progress, 100)} className="h-3" />
              <div className="grid grid-cols-3 gap-4 text-center text-sm">
                <div>
                  <div className="font-heading text-xl font-bold text-green-400">{completedToday.length}</div>
                  <div className="text-muted-foreground">Completed</div>
                </div>
                <div>
                  <div className="font-heading text-xl font-bold text-blue-400">{todayTasks.filter(t => !t.completedAt).length}</div>
                  <div className="text-muted-foreground">Remaining</div>
                </div>
                <div>
                  <div className="font-heading text-xl font-bold text-primary">{Math.round(focusTimePlanned/60)}h</div>
